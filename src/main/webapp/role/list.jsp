...
<html>
  ...
  <body>
    ...
    <div class="rightinfo">
	    ...
	    <div class="tools">
	    	<ul class="seachform">
	    		<li><label>角色名</label><form:input path="name" id="name" cssClass="scinput"/></li>
	        ...
	        <tbody>
		        <c:forEach items="${data.pageList}" var="list" varStatus="st">
		        	<tr>
				        ...
				        <td>
				        	<a href="javascript:void(0);" onclick="operate('update_state','${list.id}','${list.state }');" class="tablelink">
				        		<c:if test="${list.state==1 }">
								停用
								</c:if>
								<c:if test="${list.state==0 }">
				        	...
				        	<a href="javascript:void(0);" onclick="operate('module_role','${list.id}')" class="tablelink">编辑权限</a>    
				        	<a href="javascript:void(0);" class="tablelink" onclick="operate('deleteRole','${list.id}')"> 删除</a>
				        </td>
			        </tr>
		        </c:forEach>
	        </tbody>
